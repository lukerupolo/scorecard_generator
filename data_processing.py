...
# ================================================================================
# NEW: AI Metric Explanation Function
# ================================================================================
def get_ai_metric_explanations(metrics: list, api_key: str) -> dict:
    """
    Uses the OpenAI API to generate a simple explanation for each metric.
    """
    if not api_key: return {}
    if not metrics: return {}

    prompt = f"""
    You are a helpful marketing assistant. For each of the following metrics, provide a simple, one-sentence explanation of what it measures.
    Metrics: {json.dumps(metrics)}
    Respond *only* with a single JSON object where the keys are the metric names and the values are the one-sentence explanations.
    """
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    payload = {"model": "gpt-4-turbo", "messages": [{"role": "user", "content": prompt}], "response_format": {"type": "json_object"}, "temperature": 0.2}
    ...
    try:
        ...
        response.raise_for_status()
        result_json = response.json()
        explanations = json.loads(result_json['choices'][0]['message']['content'])
        return explanations
    except Exception:
        return {}
...
def process_scorecard_data(config: dict) -> dict:
    ...
    if not ai_categories: st.warning("Could not generate AI categories. Using 'Uncategorized'.")
    ...
    for idx, ev in enumerate(config.get('events', [])):
        ...
        for metric_name in config.get('metrics', []):
            ...
            row = {"Category": category, "Metric": metric_name, "Baseline": None, "Actual": None, "3-Month Avg": "N/A"}
