...
with st.sidebar:
    ...
    metrics = st.multiselect("Select metrics:", options=predefined_metrics, default=["Video Views (VOD)", "Hours Watched (Streams)", "Social Mentions"], key="metrics_multiselect")
    ...
    with st.spinner("Fetching data and building scorecards..."):
        for idx, ev in enumerate(events):
            ...
            for metric_name in metrics:
                ...
                # ... (This logic is condensed but functionally identical to your original version)
...
def apply_table_style_pptx(table, style_guide):
    ...
    header_font_size, body_font_size = style_guide["font_sizes"]["table_header"], style_guide["font_sizes"]["table_body"]
    ...
    # FIXED: Iterate safely over rows to avoid the slicing error
...
if st.session_state.get('show_ppt_creator'):
    ...
    with st.form("ppt_form"):
        ...
        # MOVED: Uploader is now part of this form
        uploaded_template = st.file_uploader("Upload a .pptx file to copy its style (colors, fonts, and layouts)", type="pptx")
        
        submitted = st.form_submit_button("Generate Presentation", use_container_width=True)

        if submitted:
            if not uploaded_template:
                st.error("Please upload a PowerPoint template file to continue.")
                ...
                with st.spinner("Analyzing style and building presentation..."):
                    ...
                    if style_guide:
                        template_stream = BytesIO(uploaded_template.getvalue())
                        prs = Presentation(template_stream)

                        title_slide_layout = prs.slide_layouts[0]
                        slide = prs.slides.add_slide(title_slide_layout)
                        slide.shapes.title.text, slide.placeholders[1].text = ppt_title, ppt_subtitle
