...
def apply_table_style_pptx(table, style_guide):
    primary_color, text_light = style_guide["colors"].get("primary"), style_guide["colors"].get("text_light")
    alt_bg, text_dark = style_guide["colors"].get("background_alt"), style_guide["colors"].get("text_light") # Use light text for all
...
def add_moment_title_slide(prs, title_text, style_guide):
    ...
    p = slide.shapes.title.text_frame.paragraphs[0]; p.font.name = style_guide["fonts"]["heading"]; p.font.color.rgb = style_guide["colors"]["primary"]
...
def add_timeline_slide(prs, timeline_moments, style_guide):
    ...
    fig, ax = plt.subplots(figsize=(10, 2)); fig.patch.set_facecolor('black'); ax.set_facecolor('black')
